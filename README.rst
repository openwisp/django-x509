...
Specifying x509 extensions
--------------------------

The ``extensions`` field of a certificate (or CA) accepts a JSON list of
extension entries. Each entry is an object that uses either the ``name``
key (for a small set of built-in extensions) or the ``oid`` key (for
custom private extensions). The two keys are mutually exclusive: an entry
that includes both is rejected.
...
.. code-block:: json
    ...
    [
        ...
        {"name": "extendedKeyUsage", "value": "clientAuth", "critical": true}
    ]

Custom (``oid``-based) extensions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use these to embed your own private OIDs (for example to store a device
MAC address or UUID in certificates generated by `OpenWISP Controller
<https://github.com/openwisp/openwisp-controller>`__). Each entry requires
``oid`` and ``value``; ``critical`` is optional and defaults to ``false``.

Standard and reserved X.509 extension OIDs (the ones already handled by
django-x509 or by the ``cryptography`` library, such as ``keyUsage`` or
``extendedKeyUsage``) are not allowed as custom OIDs and are rejected
during validation. Each custom OID may only appear once per certificate;
duplicate OIDs are rejected.

The ``value`` must be a string in the form ``ASN1:<TYPE>:<KIND>:<VALUE>``:

- ``<TYPE>`` is one of ``UTF8``, ``IA5``, ``PRINTABLE`` or ``OCTET``. The
  value is validated against the character set of the chosen type.
- ``<KIND>`` is either ``string`` or ``hex``. ``hex`` is only allowed with
  the ``OCTET`` type, and may use spaces, ``:`` or ``-`` as separators
  (for example ``ASN1:OCTET:hex:00:11:22``).
...
To store a MAC address and a UUID under two private OIDs, set the
certificate ``extensions`` field to:
    ...
    [
        {
            ...
            "value": "ASN1:UTF8:string:00:11:22:33:44:55"
        },
        {
            "oid": "1.3.6.1.4.1.65901.2",
            "value": "ASN1:UTF8:string:d60ecd62-5d00-4e7b-bd16-6fc64a95e60c"
        }
    ]

After saving, both values appear under the ``X509v3 extensions`` section
...
The signal is sent synchronously from within ``renew()``, which may itself
be running inside an outer database transaction. If the transaction is
later rolled back, the signal will still have been sent even though the
renewal was not persisted. Receivers that must only act once the renewal
is durably committed should defer their own logic with
