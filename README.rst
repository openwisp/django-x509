...
- Expiration notices for certificates and CAs via a daily Celery task
- Automatic renewal modes on CAs for issued certificates or for both CAs
  and issued certificates
- CRL view (public or protected)
- Possibility to specify x509 extensions on each certificate
- Expired status displayed in the certificate admin changelist
...
``DJANGO_X509_EXPIRATION_NOTICE_DAYS``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Number of days in advance used by the daily expiration task to notify that
certificates or CAs are going to expire soon.

Expiration notices and automatic renewal
----------------------------------------

Each CA now exposes an ``automatic renewal`` field in the admin with the
following choices:

- ``disabled``
- ``auto renew certificates generated by this CA``
- ``auto renew both certificates and CA``

The certificate admin changelist also includes an ``Expired`` column next
to ``Revoked`` to make the status visible without opening the object.

The base app exposes a daily Celery task which checks for expiring and
expired objects:
...
.. code-block:: python
    ...
    CELERY_BEAT_SCHEDULE = {
        "check-x509-expiration-daily": {
            "task": "django_x509.tasks.check_x509_expiration",
...
- ``django_x509.signals.x509_objects_expired`` Signature: ``(sender,
  expired_cas, expired_certs, renewed_cas, renewed_certs, failed_cas,
  failed_certs)``

The default receivers are connected in
``django_x509.apps.DjangoX509Config``. Downstream applications can
disconnect them and attach custom receivers if they need to integrate with
