...
class AutoRenewChoices(models.TextChoices):
    DISABLED = "disabled", _("disabled")
    CERTIFICATES_ONLY = (
        "certificates",
        _("auto renew certificates generated by this CA"),
    )
    CA_AND_CERTIFICATES = (
        "ca_and_certificates",
        _("auto renew both certificates and CA"),
    )
...
class BaseX509(models.Model):
    ...
    expire_notified = models.BooleanField(default=None, null=True, editable=False)
    ...
    @property
    def is_expired(self):
        return bool(self.validity_end and self.validity_end <= timezone.now())
...
class AbstractCa(BaseX509):
    ...
    auto_renew = models.CharField(
        _("automatic renewal"),
        choices=AutoRenewChoices.choices,
        default=AutoRenewChoices.DISABLED,
        max_length=24,
    )
    ...
    @property
    def can_auto_renew_certificates(self):
        return self.auto_renew in {
            AutoRenewChoices.CERTIFICATES_ONLY,
            AutoRenewChoices.CA_AND_CERTIFICATES,
        }

    @property
    def can_auto_renew_ca(self):
        return self.auto_renew == AutoRenewChoices.CA_AND_CERTIFICATES
...
class AbstractCert(BaseX509):
    ...
    def can_auto_renew(self):
        return self.ca.can_auto_renew_certificates and not self.ca.is_expired
